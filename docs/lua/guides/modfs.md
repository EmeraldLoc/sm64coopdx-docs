...
Always use `ModFs` and `ModFsFile` objects methods over regular functions. It's more clear that way and helps to reduce errors:
...
ModFS are not saved automatically when writing to files. The mod has to explicitly call the method `save` to save its ModFS on the disk.
...
ModFS files can each have their compression level explicitly set. By default this is set to 1 for minimal compression.
